...
def generate(output_path: Path = OUTPUT_PATH, cfg_override=None):
    if cfg_override is None:
        cfg_override = {}
        
    global N_NORMAL_USERS, N_INSIDER_USERS, N_DAYS, NORMAL_PHASE_DAYS, PHASED, RANDOM_SCENARIOS
    
    if 'n_normal_users' in cfg_override:
        N_NORMAL_USERS = int(cfg_override['n_normal_users'])
    if 'n_insider_users' in cfg_override:
        N_INSIDER_USERS = int(cfg_override['n_insider_users'])
    if 'n_days' in cfg_override:
        N_DAYS = int(cfg_override['n_days'])
    if 'normal_phase_days' in cfg_override:
        NORMAL_PHASE_DAYS = int(cfg_override['normal_phase_days'])
    if 'phased' in cfg_override:
        PHASED = bool(cfg_override['phased'])
    if 'random_scenarios' in cfg_override:
        RANDOM_SCENARIOS = bool(cfg_override['random_scenarios'])
